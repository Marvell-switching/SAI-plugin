...
sai_status_t mrvl_sai_neighbor_dump(void)
{
    ...
    int chars_written;
    printf("\nNeighbor table:\n");
    printf("| idx | next nbr id |  dst mac address  |  ipv4 address   |\n");
    printf("-----------------------------------------------------------\n");
    for (idx = 0; idx < SAI_NEIGHBOR_TABLE_SIZE_CNS; idx++) {
        if (mrvl_sai_nbr_table[idx].used == true) {
            ...
            printf("| %3d |    %8x | %02x:%02x:%02x:%02x:%02x:%02x | %15s |\n", idx, 
                                        ...
                                        _In_ const sai_attribute_t      *attr_list)
{
    ...
    if (neighbor_entry->ip_address.addr_family != SAI_IP_ADDR_FAMILY_IPV4) {
        MRVL_SAI_LOG_ERR("Ip address family is not supported %d\n", neighbor_entry->ip_address.addr_family);
        return SAI_STATUS_NOT_SUPPORTED;
    }
    ...
    status = mrvl_sai_next_hop_update_nbr_id(rif_idx, nbr_idx, &mrvl_sai_nbr_table[nbr_idx].inet_address);
...
sai_status_t mrvl_sai_remove_neighbor_entry(_In_ const sai_neighbor_entry_t* neighbor_entry)
{
    sai_status_t    status;
    char            key_str[MAX_KEY_STR_LEN];
    uint32_t        rif_idx, nbr_idx, rif_first_nbr, new_first_idx;
    ...
    status = mrvl_sai_next_hop_update_nbr_id(rif_idx, MRVL_SAI_INVALID_ID_CNS, &mrvl_sai_nbr_table[nbr_idx].inet_address);
    if ((status != SAI_STATUS_SUCCESS) && (status != SAI_STATUS_ITEM_NOT_FOUND)) {
        MRVL_SAI_LOG_ERR("Can't update neighbor\n");
        return SAI_STATUS_FAILURE;
    }
                                         ...
                                         _In_ const sai_attribute_t      *attr)
{
    ...
    mrvl_sai_neighbor_key_to_str(neighbor_entry, key_str);
    return mrvl_sai_utl_set_attribute(&key, key_str, mrvl_sai_neighbor_attribs, mrvl_sai_neighbor_vendor_attribs, attr);
}
                                         ...
                                         _Inout_ sai_attribute_t         *attr_list)
{
    ...
    mrvl_sai_neighbor_key_to_str(neighbor_entry, key_str);
    return mrvl_sai_utl_get_attributes(&key, key_str, mrvl_sai_neighbor_attribs, mrvl_sai_neighbor_vendor_attribs, attr_count, attr_list);
}
...
{
    sai_status_t         status;
    uint32_t             rif_idx, nbr_idx, delete_idx;
