                                          ...
                                          _Inout_ sai_attribute_t    *attr_list)
{
    ...
    mrvl_sai_fdb_key_to_str_prv(fdb_entry, key_str);
    return mrvl_sai_utl_get_attributes(&key, key_str, mrvl_sai_fdb_attribs, mrvl_sai_fdb_vendor_attribs, attr_count, attr_list);
}
...
sai_status_t mrvl_sai_set_fdb_entry_attribute(_In_ const sai_fdb_entry_t* fdb_entry, _In_ const sai_attribute_t *attr)
{
    ...
    mrvl_sai_fdb_key_to_str_prv(fdb_entry, key_str);
    return mrvl_sai_utl_set_attribute(&key, key_str, mrvl_sai_fdb_attribs, mrvl_sai_fdb_vendor_attribs, attr);
}
...
sai_status_t mrvl_sai_flush_fdb_entries(_In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list)
{
    ...
    uint32_t       				first_entry = 1, count = 0;
    FPA_FLOW_TABLE_ENTRY_STC	fpa_flow_entry;
    ...
    if (SAI_STATUS_SUCCESS ==
        (status = mrvl_sai_utl_find_attrib_in_list(attr_count, attr_list, SAI_FDB_FLUSH_ATTR_ENTRY_TYPE,
                                 &type, &type_index))) {
        MRVL_SAI_LOG_ERR("Entry type is not supported\n");
    }

    while (fpa_status == FPA_OK) {
        fpa_status = fpaLibFlowTableGetNext(SAI_DEFAULT_ETH_SWID_CNS, FPA_FLOW_TABLE_TYPE_L2_BRIDGING_E, first_entry, &fpa_flow_entry);
        if (fpa_status == FPA_OK) {
            if (first_entry == 1) {
                first_entry = 0;
            }
            if ((vlan_exist) && (fpa_flow_entry.data.l2_bridging.match.vlanId != vlan->u16)){
                continue;
            }
            if ((port_exist) && (fpa_flow_entry.data.l2_bridging.groupId != 0xFFFFFFFF)){
                fpaLibGroupIdentifierParse(fpa_flow_entry.data.l2_bridging.groupId, &parsed_group_identifier);
                if (parsed_group_identifier.portNum !=port_id) {
                    continue; 
                }
            }
            fpa_status = fpaLibFlowTableCookieDelete(SAI_DEFAULT_ETH_SWID_CNS, FPA_FLOW_TABLE_TYPE_L2_BRIDGING_E, fpa_flow_entry.cookie); 
            count++;
        }
    }
...
sai_status_t mrvl_sai_fdb_event_convert_fpa_to_sai(FPA_EVENT_ADDRESS_MSG_STC *fpa_au_event, sai_fdb_event_notification_data_t *fdb_event)
{
    ...
    if (SAI_STATUS_SUCCESS != mrvl_sai_utl_create_object(SAI_OBJECT_TYPE_PORT, fpa_au_event->portNum, &(fdb_event->attr[0].value.oid)))
    {
