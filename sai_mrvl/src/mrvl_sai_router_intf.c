...
#define MRVL_SAI_ROUTER_INTF_GET_FROM_COOKIE_MAC(cookie, type, portVlan, macAddr)\
        type =     ((uint64_t)cookie >> 63) & 0x1; \
        portVlan = ((uint64_t)cookie >>48)& 0x7fff; \
        macAddr[0] = ((uint64_t)cookie >> 40)&0xff; \
        macAddr[1] = ((uint64_t)cookie >>32)&0xff; \
        macAddr[2] = ((uint64_t)cookie >>24)&0xff; \
        macAddr[3] = ((uint64_t)cookie >>16)&0xff; \
        macAddr[4] = ((uint64_t)cookie >>8)&0xff; \
        macAddr[5] = ((uint64_t)cookie)&0xff
    ...
    for (idx = 0; idx < SAI_ROUTER_INTERFACE_TABLE_SIZE_CNS; idx++) {
        if (mrvl_sai_rif_table[idx].valid == true) {
            ...
            printf("| %3d | %7d |   %s  | %5d |  %d |   %s   | %8x | %s |   %s  | %8x |%02x:%02x:%02x:%02x:%02x:%02x| ", idx, mrvl_sai_rif_table[idx].mtu_idx,
                   (mrvl_sai_rif_table[idx].intf_type == SAI_ROUTER_INTERFACE_TYPE_PORT)?"P":"V",
...
sai_status_t mrvl_sai_remove_router_interface(_In_ sai_object_id_t rif_id)
{
    ...
    if (fpa_status != FPA_OK) {
        MRVL_SAI_LOG_ERR("Failed to delete entry %llx from TERMINATION table status = %d\n", cookie, fpa_status);
...
{
    sai_status_t                 status;
    const sai_attribute_value_t  *type, *vrid, *port, *vlan, *mtu, *mac, *adminv4, *miss_act;
    ...
    char                         key_str[MAX_KEY_STR_LEN];
    bool                         ipv4_enable, found_empty, vr_is_valid;
    ...
    if (rif_idx == 0xFFFFFFFF) { /*mtu size table is full */
    ...
    cookie = MRVL_SAI_ROUTER_INTF_CREATE_COOKIE_MAC(type->s32, portVlan, mac_addr);
                                                 ...
                                                 _In_ const sai_attribute_t    *attr)
{
    ...
    mrvl_sai_rif_key_to_str(rif_id, key_str);
    return mrvl_sai_utl_set_attribute(&key, key_str, mrvl_sai_rif_attribs, mrvl_sai_rif_vendor_attribs, attr);
}
                                                 ...
                                                 _Inout_ sai_attribute_t       *attr_list)
{
    ...
    mrvl_sai_rif_key_to_str(rif_id, key_str);
    return mrvl_sai_utl_get_attributes(&key, key_str, mrvl_sai_rif_attribs, mrvl_sai_rif_vendor_attribs, attr_count, attr_list);
}
                                 ...
                                 void                          *arg)
{
    ...
    case SAI_ROUTER_INTERFACE_ATTR_PORT_ID:     
        ...
        if (SAI_STATUS_SUCCESS != mrvl_sai_utl_create_object(SAI_OBJECT_TYPE_PORT, flowEntry.data.termination.match.inPort, &value->oid)) {
            return SAI_STATUS_FAILURE;
        }
        break;
    ...
    case SAI_ROUTER_INTERFACE_ATTR_VLAN_ID:
        ...
        value->u16 = flowEntry.data.termination.match.vlanId;
        break;
    ...
    case SAI_ROUTER_INTERFACE_ATTR_SRC_MAC_ADDRESS:
        memcpy(value->mac, &flowEntry.data.termination.match.destMac, 6);
        break;
