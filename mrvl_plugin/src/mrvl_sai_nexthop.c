...
sai_status_t mrvl_sai_next_hop_dump(void)
{
    ...
    printf("\nNext hop table:\n");
    printf("| idx | next nh id | action | neighbor id | rif id | rout_v |  ipv4 address  | route list\n");
    printf("-----------------------------------------------------------------------------------------------\n");
                                  ...
                                  _In_ const sai_attribute_t *attr_list)
{
    ...
    if (SAI_IP_ADDR_FAMILY_IPV4 != ip->ipaddr.addr_family) {
        /* TODO : implement IPv6 */
        MRVL_SAI_LOG_ERR("IPv6 not implemented\n");
 ...
 *    Failure status code on error
 */
sai_status_t mrvl_sai_next_hop_update_nbr_id(uint32_t rif_idx, uint32_t nbr_idx, sai_ip_address_t *inet_address)
{
...
sai_status_t mrvl_sai_set_next_hop_attribute(_In_ sai_object_id_t next_hop_id, _In_ const sai_attribute_t *attr)
{
    ...
    mrvl_sai_next_hop_key_to_str(next_hop_id, key_str);
    return mrvl_sai_utl_set_attribute(&key, key_str, mrvl_sai_next_hop_attribs, mrvl_sai_next_hop_vendor_attribs, attr);
}
                                         ...
                                         _Inout_ sai_attribute_t *attr_list)
{
    ...
    mrvl_sai_next_hop_key_to_str(next_hop_id, key_str);
    return mrvl_sai_utl_get_attributes(&key, key_str, mrvl_sai_next_hop_attribs, mrvl_sai_next_hop_vendor_attribs, attr_count, attr_list);
}
