                               ...
                               _In_ const sai_attribute_t           *attr_list)
{
    ...
    uint32_t                     nh_idx, nbr_idx, rif_idx, rif_vr_id;
    bool                         vr_is_valid;
    ...
    if (unicast_route_entry->destination.addr_family != SAI_IP_ADDR_FAMILY_IPV4) {
        MRVL_SAI_LOG_ERR("addr_family %d is not supported\n", unicast_route_entry->destination.addr_family);
        return SAI_STATUS_NOT_SUPPORTED;
    }
    ...
    }else if ((entry.data.nh_type == SAI_OBJECT_TYPE_ROUTER_INTERFACE) &&
        (entry.data.nh_valid == true)){
...
sai_status_t mrvl_sai_route_remove_all(void)
{
    ...
    while (entry_ptr != NULL) {
        ...
        memcpy(&key, &entry_ptr->key, sizeof(mrvl_sai_route_hash_key_t));	
        if (entry_ptr->key.destination.mask.ip4 != 0) { /* we can't delete default entry*/
                                      ...
                                      _In_ const sai_attribute_t           *attr)
{
    ...
    mrvl_sai_route_key_to_str(unicast_route_entry, key_str);
    return mrvl_sai_utl_set_attribute(&key, key_str, mrvl_sai_route_attribs, mrvl_sai_route_vendor_attribs, attr);
}
                                      ...
                                      _Inout_ sai_attribute_t              *attr_list)
{
    ...
    mrvl_sai_route_key_to_str(unicast_route_entry, key_str);
    return mrvl_sai_utl_get_attributes(&key, key_str, mrvl_sai_route_attribs, mrvl_sai_route_vendor_attribs, attr_count, attr_list);
}
