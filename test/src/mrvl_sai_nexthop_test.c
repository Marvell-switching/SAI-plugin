...
)
{
    ...
    sai_uint32_t vlan_member_list[3];
    ...
    ports_list[num_of_ports++].tag = SAI_VLAN_PORT_TAGGED;
    mrvl_sai_wrap_vlan_add_ports_list(vlan, num_of_ports, ports_list, vlan_member_list);
...
)
{
    ...
    attr_list[attr_count].id = SAI_NEXT_HOP_ATTR_IP;
    for (i=0; i<16; i++) {
        attr_list[attr_count].value.ipaddr.addr.ip6[i] = ipv6.s6_addr[i];
    }
    attr_list[attr_count].value.ipaddr.addr_family = SAI_IP_ADDR_FAMILY_IPV6;
    attr_count++;
